...
from .forms import RegistrationForm, BookingForm, EmailLoginForm, AdminLoginForm
...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            user = form.save()
            user.backend = 'carforuser.CustomAuthBackend'
            login(request, user, backend='carforuser.CustomAuthBackend')
...
def admin_login_view(request):
    ...
    if request.method == 'POST':
        if form.is_valid():
            ...
            if user is not None:
                login(request, user)
                return redirect('admin_panel')  # Убедитесь, что перенаправление на правильный URL
            else:
...
def booking_dashboard(request):
    ...
    return render(request, 'booking_dashboard.html', {'bookings': bookings})
...
def admin_panel(request):
    if not request.user.is_superuser:
        messages.error(request, 'У вас недостаточно прав для доступа к этой странцие')
